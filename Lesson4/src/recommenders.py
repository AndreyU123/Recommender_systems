...
class MainRecommender:
    ...
    def __init__(self, data, item_features, weighting=True):
        ...
        self.item_id_to_ctm = self.prepare_to_ctm_dict(self.item_features, self.user_item_matrix)  # your_code
    ...
    @staticmethod
    def prepare_to_ctm_dict(item_features, user_item_matrix):
    ...
    @staticmethod
    def fit(useritemmatrix, n_factors=20, regularization=0.001, iterations=15, num_threads=4):
        """Обучает ALS"""
    ...
    def get_rec(self, itemid, N):
        recs = self.model.similar_items(self.itemid_to_id[itemid], N=N)
        top_rec = recs[1][0]
